import { fourSum, fourSumPointers } from "./fourSum";

describe("fourSum bruteforce solution", () => {
  ...
  it("simple case nums =[2,1,4,3,6,5], target = 0", () => {
    const nums = [2, 1, 4, 3, 6, 5];
    const target = 12;
    const expected = [[1,2,3,6], [1,2,4,5] ];
    expect(fourSum(nums, target)).toEqual(expected);
  });
...
describe("fourSumPointers", () => {
  it("simple case nums = [1,0,-1,0,-2,2], target = 0", () => {
    ...
    expect(fourSumPointers(nums, target)).toEqual(expected);
  });
  it("simple case nums = [2,2,2,2,2], target = 8", () => {
    const nums = [2, 2, 2, 2, 2];
    const target = 8;
